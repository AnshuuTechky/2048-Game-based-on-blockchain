...
const { ethers } = require("ethers"); // Make sure to have ethers.js included in your HTML
const provider = new ethers.providers.Web3Provider(window.ethereum);
let signer;
let contract; // Replace with your contract's address and ABI
...
// Function to mint token with the highest score
const mintWithHighScore = async (score) => {
    if (window.ethereum) {
        ...
        try {
            ...
            await tx.wait();
            alert("Token minted successfully with score: " + score);
        } catch (error) {
